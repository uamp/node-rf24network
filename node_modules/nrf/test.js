...
//console.log(nrf.isValid());
//nrf._debug = true;
nrf.channel(88).transmitPower('PA_MAX').dataRate('250kbps').crcBytes(2).autoRetransmit({count:15, delay:4000});
nrf.begin(function () {
    if (role === 'ping') {
		...
		setInterval(function(){
			var b= new Buffer(4);
			b.writeUInt32BE(sim_data++,0); 
			tx.write(b);
			console.log("Writing ",sim_data);
		 },1000);
        });
	tx.on('error',function (e) {console.log("Error",e)});
    } else {
        console.log("PONG back");
        //var rx = nrf.openPipe('rx', pipes[0],{size:20, autoAck:true});
